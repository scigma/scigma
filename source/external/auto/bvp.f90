...
! ---------- ------
  SUBROUTINE CNRLBV(AC,ICP,ICU,FUNI,BCNI,ICNI,STPNBVI,FNCI)
    ...
    USE IO
    USE MESH
    USE SUPPORT, ONLY: CHECKSP, STOPPED, &
...
! INITIALIZE COMPUTATION OF BRANCH
    ...
    ALLOCATE(TM(0:NTST),AC%SUPPORT%DTV(NTST))
    ALLOCATE(AC%SUPPORT%P0V(NDIM,NDIM),AC%SUPPORT%P1V(NDIM,NDIM),&
         TEST(AP%NTEST),AC%SUPPORT%EVV(AP%NDM))
    ...
    CALL RSPTBV(AC,PAR,ICP,FUNI,STPNBVI,FNCI,RLCUR,RLOLD,RLDOT, &
         NDIM,UPS,UOLDPS,UDOTPS,UPOLDP,TM,AC%SUPPORT%DTV,NODIR,THU)

!     don't set global rotations here for homoclinics, but in autlib5.f
    IF(IPS.NE.9)CALL SETRTN(AC,AP%NDM,NTST*NCOL,NDIM,UPS)
    ...
    IF(IPERP/=2)THEN
       CALL STDRBV(AC,PAR,ICP,FUNI,BCNI,ICNI,RLCUR,RLOLD,RLDOT, &
            NDIM,UPS,UOLDPS,UDOTPS,UPOLDP,AC%SUPPORT%DTV,IPERP,&
            AC%SUPPORT%P0V,AC%SUPPORT%P1V,THL,THU)
...
! Store plotting data for restart point :
    ...
    CALL STPLBV(AC,PAR,ICP,ICU,RLDOT,NDIM,UPS,UDOTPS,TM,AC%SUPPORT%DTV,THU,ISTOP)
    DO WHILE(ISTOP==0)
       ITP=0
       AP%ITP=ITP
       NINS=AP%NINS
       CALL STEPBV(AC,DSOLD,PAR,ICP,FUNI,BCNI,ICNI,FNCI,RDS, &
            ...
            TM,AC%SUPPORT%DTV,AC%SUPPORT%P0V,AC%SUPPORT%P1V,THL,THU,NITPS,ISTOP)
       ...
       ELSE
          ...
          DO ITEST=1,AP%NTEST
             ...
             CALL LCSPBV(AC,DSOLD,DSTEST,PAR,ICP,ITEST,FUNI,BCNI,ICNI,FNCI, &
                  ...
                  UPOLDP,TM,AC%SUPPORT%DTV,AC%SUPPORT%P0V,AC%SUPPORT%P1V,&
                  AC%SUPPORT%EVV,THL,THU,IUZ,VUZ,NITPS,ATYPE,STEPPED)
...
! Adapt the mesh to the solution.
       ...
       IF(IAD.NE.0)THEN
          IF(MOD(NTOT,IAD).EQ.0)THEN
             CALL ADAPT(NTST,NCOL,NDIM,TM,AC%SUPPORT%DTV,UPS,UOLDPS, &
...
! Update UOLDPS, UDOTPS, UPOLDP, RLOLD, and RLDOT.
       ...
       CALL CONTBV(AC,DSOLD,PAR,ICP,FUNI,RLCUR,RLOLD,RLDOT, &
            NDIM,UPS,UOLDPS,UDOTPS,UPOLDP,AC%SUPPORT%DTV,THL,THU)

    ENDDO
    DEALLOCATE(PAR,THL,THU,IUZ,VUZ)
    DEALLOCATE(AC%SUPPORT%EVV,UPS,UOLDPS,UPOLDP,UDOTPS,TM,&
         AC%SUPPORT%DTV,AC%SUPPORT%P0V,AC%SUPPORT%P1V)
...
! ---------- ------
  SUBROUTINE SETRTN(AC,NDM,NTNC,NDIM,UPS)
...
! Initialization for rotations
    ...
    ALLOCATE(AC%BVP%NRTN(NDM))
    AC%BVP%IRTN=0
    DO I=1,NDM
       AC%BVP%NRTN(I)=NINT( (UPS(I,NTNC)-UPS(I,0)) / PI(2.d0) )
       IF(AC%BVP%NRTN(I).NE.0)THEN
          AC%BVP%IRTN=1
       ENDIF
    ENDDO
    IF(AC%BVP%IRTN.EQ.0)DEALLOCATE(AC%BVP%NRTN)
