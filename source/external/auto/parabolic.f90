...
! Redefinition for waves
    ...
    CASE(12) 
       ...
       CALL INITPS(AP,ICP)
       IF(ABS(ISW)<=1) THEN
          CALL AUTOBV(AC,ICP,ICU,FNWP,BCPS,ICPS,STPNPS,FNCSPS)
       ELSE IF(ABS(ISW)==2) THEN
          IF(ITP==5) THEN 
             ! ** Fold continuation (start).
             CALL AUTOBV(AC,ICP,ICU,FNWPL,BCPL,ICPL,STPNPL,FNCSPS)
          ELSE IF(ABS(ITP)/10==5) THEN
             ! ** Fold continuation (restart).
             CALL AUTOBV(AC,ICP,ICU,FNWPL,BCPL,ICPL,STPNBV,FNCSPS)
          ENDIF
       ENDIF
    CASE(14) 
       ! ** Evolution calculations for parabolic systems.
       !    (Periodic boundary conditions.)
       CALL AUTOBV(AC,ICP,ICU,FNPE,BCPS,ICPE,STPNBV,FNCSPE)
    CASE(16)
       ! ** Evolution calculations for parabolic systems.
       !    (User supplied boundary conditions.)
       CALL AUTOBV(AC,ICP,ICU,FNPE,BCNI,ICPE,STPNBV,FNCSPE)
    CASE(17)
       ! ** Continuation of stationary states of parabolic systems.
       !    (User supplied boundary conditions.)
       CALL AUTOBV(AC,ICP,ICU,FNSP,BCNI,ICPE,STPNBV,FNCSBV)
    END SELECT
  END SUBROUTINE AUTOPE

! ---------- ----
  SUBROUTINE FNWS(AC,NDIM,U,UOLD,ICP,PAR,IJAC,F,DFDU,DFDP)
...
! ------ --------- -------- ------
  DOUBLE PRECISION FUNCTION FNCSWS(AC,ICP,U,NDIM,PAR,ITEST,ATYPE) RESULT(Q)
    ...
    Q=FNCSEQF(AC,ICP,U,NDIM,PAR,ITEST,ATYPE,FNWS)

  END FUNCTION FNCSWS

! ---------- ----
  SUBROUTINE FNWL(AC,NDIM,U,UOLD,ICP,PAR,IJAC,F,DFDU,DFDP)
    ...
    CALL FNLPF(AC,NDIM,U,UOLD,ICP,PAR,IJAC,F,DFDU,DFDP,FNWS)

  END SUBROUTINE FNWL

! ---------- -------
  SUBROUTINE STPNWL(AC,PAR,ICP,U,UDOT,NODIR)
    ...
    CALL STPNLPF(AC,PAR,ICP,U,UDOT,NODIR,FNWS)

  END SUBROUTINE STPNWL

! ---------- ----
  SUBROUTINE FNHW(AC,NDIM,U,UOLD,ICP,PAR,IJAC,F,DFDU,DFDP)
    ...
    CALL FNHBF(AC,NDIM,U,UOLD,ICP,PAR,IJAC,F,DFDU,DFDP,FNWS,FFHBX)

  END SUBROUTINE FNHW

! ---------- ------
  SUBROUTINE STPNHW(AC,PAR,ICP,U,UDOT,NODIR)
    ...
    CALL STPNHBF(AC,PAR,ICP,U,UDOT,NODIR,FNWS,FFHBX)

  END SUBROUTINE STPNHW

! ---------- ----
  SUBROUTINE FNWP(AC,NDIM,U,UOLD,ICP,PAR,IJAC,F,DFDU,DFDP)
    ...
    CALL FNWS(AC,NDIM,U,UOLD,ICP,PAR,ABS(IJAC),F,DFDU,DFDP)
    PERIOD=PAR(11)
...
! ---------- -----
  SUBROUTINE FNWPL(AC,NDIM,U,UOLD,ICP,PAR,IJAC,F,DFDU,DFDP)
    ...
    CALL FNPLF(AC,NDIM,U,UOLD,ICP,PAR,IJAC,F,DFDU,DFDP,FNWP)
...
! ---------- ----
  SUBROUTINE FNSP(AC,NDIM,U,UOLD,ICP,PAR,IJAC,F,DFDU,DFDP)
    ...
    CALL FUNI(AC,NDM,U,UOLD,ICP,PAR,IJAC,F(NDM+1),DFDU,DFDP)
...
! ---------- ----
  SUBROUTINE FNPE(AC,NDIM,U,UOLD,ICP,PAR,IJAC,F,DFDU,DFDP)
    ...
    CALL FUNI(AC,NDM,U,UOLD,ICP,PAR,IIJAC,F(NDM+1),DFDU,DFDP)
...
! ---------- ----
  SUBROUTINE ICPE(AC,NDIM,PAR,ICP,NINT,U,UOLD,UDOT,UPOLD,F,IJAC,DINT)
...
! ------ --------- -------- ------
  DOUBLE PRECISION FUNCTION FNCSPE(AC,ICP,UPS,NDIM,PAR,ITEST,ATYPE) RESULT(Q)
