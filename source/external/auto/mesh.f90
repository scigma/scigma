...
! ---------- ------
  SUBROUTINE ADPTDS(AC,NIT,ITNW,IBR,NTOP,IID,DSMAX,RDS)
...
! number of Newton iterations in the previous step (called if IADS > 0).
    ...
    IF(IID>0)THEN
       WRITE(AC%DUNIT,"(/,I4,I6,8X,A,I3)")ABS(IBR),NTOP,' Iterations   : ',NIT
       WRITE(AC%DUNIT,"(I4,I6,8X,A,ES13.5)")ABS(IBR),NTOP,' Next Step    : ',RDS
    ENDIF
