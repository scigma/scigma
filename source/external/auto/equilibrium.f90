...
! ------ --------- -------- -------
  DOUBLE PRECISION FUNCTION FNCSEQF(AC,ICP,U,NDIM,PAR,ITEST,ATYPE,FUNI) &
    ...
    CASE(4) ! Check for Bogdanov-Takens bifurcation
       Q=FNBTEQ(AC,ATYPE,U,AC%SUPPORT%P0V)
    CASE(5) ! Check for generalized Hopf (Bautin)
       Q=FNGHEQ(AC,PAR,ICP,ATYPE,FUNI,U,AC%SUPPORT%P0V)
    CASE(6) ! Check for Hopf or Zero-Hopf
       Q=FNHBEQ(AC,PAR,ATYPE,AC%SUPPORT%P0V)
    END SELECT
...
! ---------- --------
  SUBROUTINE PRINTEIG(AC)
    ...
    IF(AP%IID>0)THEN
       ...
       DO i=1,AP%NDM
          WRITE(AC%DUNIT,103)ABS(AP%IBR),NTOP+1,I,AC%SUPPORT%EVV(i)
       ENDDO
    ENDIF
...
! ------ --------- -------- ------
  DOUBLE PRECISION FUNCTION FNHBEQ(AC,PAR,ATYPE,AA)
    ...
    USE SUPPORT, ONLY: PI, EIG, CHECKSP, LBTYPE
