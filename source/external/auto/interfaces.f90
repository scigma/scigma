...
!-----------------------------------------------------------------------
      ...
      INTERFACE
         SUBROUTINE FUNC(ID,NDIM,U,ICP,PAR,IJAC,F,DFDU,DFDP)
         INTEGER, INTENT(IN) :: ID,NDIM, IJAC, ICP(*)
         DOUBLE PRECISION, INTENT(IN) :: U(NDIM), PAR(*)
         DOUBLE PRECISION, INTENT(OUT) :: F(NDIM)
         DOUBLE PRECISION, INTENT(INOUT) :: DFDU(NDIM,NDIM),DFDP(NDIM,*)
         END SUBROUTINE FUNC

         SUBROUTINE STPNT(ID,NDIM,U,PAR,T)
         INTEGER, INTENT(IN) :: ID,NDIM
         DOUBLE PRECISION, INTENT(INOUT) :: U(NDIM),PAR(*)
         DOUBLE PRECISION, INTENT(IN) :: T
         END SUBROUTINE STPNT

         SUBROUTINE BCND(ID,NDIM,PAR,ICP,NBC,U0,U1,FB,IJAC,DBC)
         INTEGER, INTENT(IN) :: ID,NDIM, ICP(*), NBC, IJAC
         DOUBLE PRECISION, INTENT(IN) :: PAR(*), U0(NDIM), U1(NDIM)
         DOUBLE PRECISION, INTENT(OUT) :: FB(NBC)
         DOUBLE PRECISION, INTENT(INOUT) :: DBC(NBC,*)
         END SUBROUTINE BCND

         SUBROUTINE ICND(ID,NDIM,PAR,ICP,NINT,U,UOLD,UDOT,UPOLD,FI,IJ,DINT)
         INTEGER, INTENT(IN) :: ID,NDIM, ICP(*), NINT, IJ
...
! Generate the Jacobian by differencing.
         ...
         DO I=1,NDIM
           UU=U(I)
           U(I)=UU-EP
           CALL FUNC(AC%ID,NDIM,U,ICP,PAR,0,F,DFDU,DFDP)
           U(I)=UU+EP
           CALL FUNC(AC%ID,NDIM,U,ICP,PAR,0,DFDU(1,I),DFDU,DFDP)
           U(I)=UU
           DO J=1,NDIM
       ...
       ELSE
         ...
         ENDIF
       ENDIF
       CALL FUNC(AC%ID,NDIM,U,ICP,PAR,IJC,F,DFDU,DFDP)
       ...
       DO I=1,NFPR
         ...
         EP=HMACH*( 1 +ABS(P) )
         PAR(ICP(I))=P+EP
         CALL FUNC(AC%ID,NDIM,U,ICP,PAR,0,DFDP(1,ICP(I)),DFDU,DFDP)
         DO J=1,NDIM
...
! Generate the Jacobian by differencing.
          ...
          DO I=1,NDIM
             UU=U0(I)
             U0(I)=UU-EP
             CALL BCND(AC%ID,NDIM,PAR,ICP,NBC,U0,U1,F,0,DBC)
             U0(I)=UU+EP
             CALL BCND(AC%ID,NDIM,PAR,ICP,NBC,U0,U1,DBC(1,I),0,DBC)
             U0(I)=UU
             DO J=1,NBC
          ...
          DO I=1,NDIM
             ...
             CALL BCND(AC%ID,NDIM,PAR,ICP,NBC,U0,U1,F,0,DBC)
             U1(I)=UU+EP
             CALL BCND(AC%ID,NDIM,PAR,ICP,NBC,U0,U1,DBC(1,NDIM+I),0,DBC)
             U1(I)=UU
             DO J=1,NBC
       ...
       ELSE
         ...
         ENDIF
       ENDIF
       CALL BCND(AC%ID,NDIM,PAR,ICP,NBC,U0,U1,F,IJC,DBC)
       IF(JAC==1 .OR. IJAC/=2)RETURN
       ...
       DO I=1,NFPR
         ...
         EP=HMACH*( 1 +ABS(P))
         PAR(ICP(I))=P+EP
         CALL BCND(AC%ID,NDIM,PAR,ICP,NBC,U0,U1,DBC(1,2*NDIM+ICP(I)),0,DBC)
         DO J=1,NBC
...
! Generate the Jacobian by differencing.
          ...
          DO I=1,NDIM
             UU=U(I)
             U(I)=UU-EP
             CALL ICND(AC%ID,NDIM,PAR,ICP,NINT,U,UOLD,UDOT,UPOLD,F,0,DINT)
             U(I)=UU+EP
             CALL ICND(AC%ID,NDIM,PAR,ICP,NINT,U,UOLD,UDOT,UPOLD,DINT(1,I),0,DINT)
             U(I)=UU
             DO J=1,NINT
       ...
       ELSE
         ...
         ENDIF
       ENDIF
       CALL ICND(AC%ID,NDIM,PAR,ICP,NINT,U,UOLD,UDOT,UPOLD,F,IJC,DINT)
       IF(JAC==1 .OR. IJAC/=2)RETURN
       ...
       DO I=1,NFPR
         ...
         EP=HMACH*( 1 +ABS(P) )
         PAR(ICP(I))=P+EP
         CALL ICND(AC%ID,NDIM,PAR,ICP,NINT,U,UOLD,UDOT,UPOLD, &
...
! ---------- -----
      SUBROUTINE PVLSI(AC,UPS,NDIM,PAR)
