...
! ---------- -----
  SUBROUTINE FFNSX(AC,U,PAR,DFDU,DFDV)
    ...
    ENDDO
    F=2*SIN(THTA)
    CALL DGEMV('n',NDM,NDM,F,DFDU,NDM,U(NDM+1),1,0.d0,DFDV(1,NDM+1),1)
...
! ------ --------- -------- ------
  DOUBLE PRECISION FUNCTION FNCSDS(AC,ICP,U,NDIM,PAR,ITEST,ATYPE) RESULT(Q)
    ...
    SELECT CASE(ITEST)
    CASE(4)
       Q=FNRNDS(AC,ATYPE,U,AC%SUPPORT%P0V)
    CASE(6)
       Q=FNHBDS(AC,PAR,ATYPE,AC%SUPPORT%P0V)
    CASE DEFAULT
