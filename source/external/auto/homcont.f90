  ...
  SUBROUTINE FBHO(AC,NDIM,PAR,ICP,NBC,CSAVE,U0,U1,FB)
    ...
    IF(AC%HOMCONT%ISTART/=3) THEN
       ...
       IF ((AC%HOMCONT%IEQUIB/=0).AND.(AC%HOMCONT%IEQUIB/=-1)) THEN
          CALL FUNC(AC%ID,NDM,XEQUIB1,ICP,PAR,0,FB(JB),DUM1,DUM2)
          JB=JB+NDM
          !    *NDM extra initial conditions for the equilibrium if IEQUIB=-2
          IF (AC%HOMCONT%IEQUIB==-2) THEN
             CALL FUNC(AC%ID,NDM,XEQUIB2,ICP,PAR,0,FB(JB),DUM1,DUM2)
             JB=JB+NDM
          ENDIF
       ENDIF
    ...
    IF (NBCN>0) THEN
       CALL BCND(AC%ID,NDIM,PAR,ICP,NBCN,U0,U1,FB(JB),0,DUM2)
    ELSEIF (NBCN<0) THEN
...
! ---------- ------
  SUBROUTINE TRANHO(AC,NTSR,NCOLRS,NDM,NDIM,TM,UPS,UDOTPS,PAR,ICP,NPAR)
    ...
    ENDIF
    TMMAX=TM(JMAX)
    CALL FUNC(AC%ID,NDM,UPS(1,JMAX*NCOLRS),ICP,PAR,0,PAR(NPAR-NDM+1),DUM1,DUM2)
  ...
  SUBROUTINE PREHO(AC,PAR,ICP,NTSR,NAR,NCOLRS,UPS,UDOTPS,TM)
    ...
    IF (AC%HOMCONT%ISTART==4) THEN
       ...
       IF (AC%HOMCONT%IEQUIB>0) THEN
          ...
          JMIN=0
          DO J=0,NTSR*NCOLRS
             CALL FUNC(AC%ID,NDM,UPS(1,J),ICP,PAR,0,F,DUM1,DUM2)
             UPSI=0
             DO I=1,NDM
                UPSI=UPSI+F(I)*F(I)
             ENDDO
...
! ------ --------- -------- -----
  DOUBLE PRECISION FUNCTION PSIHO(AC,NDM,IS,RR,RI,V,VT,ICP,PAR,PU0,PU1)
    ...
    CALL FUNC(AC%ID,NDM,PU0,ICP,PAR,0,F0,DUM1,DUM2)
    CALL FUNC(AC%ID,NDM,PU1,ICP,PAR,0,F1,DUM1,DUM2)
