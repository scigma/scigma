...
void CTwGraphOpenGLCore::ResizeTriBuffers(size_t _NewSize)
{
    ...
    _glBufferData(GL_ARRAY_BUFFER, GLsizeiptr(m_TriBufferSize*sizeof(Vec2)), 0, GL_DYNAMIC_DRAW);

    _glBindBuffer(GL_ARRAY_BUFFER, m_TriUVs);
    _glBufferData(GL_ARRAY_BUFFER, GLsizeiptr(m_TriBufferSize*sizeof(Vec2)), 0, GL_DYNAMIC_DRAW);

    _glBindBuffer(GL_ARRAY_BUFFER, m_TriColors);
    _glBufferData(GL_ARRAY_BUFFER, GLsizeiptr(m_TriBufferSize*sizeof(color32)), 0, GL_DYNAMIC_DRAW);
...
void CTwGraphOpenGLCore::EndDraw()
{
    ...
    _glBlendFunc(GLuint(m_PrevSrcBlend), GLuint(m_PrevDstBlend)); CHECK_GL_ERROR;

    _glBindTexture(GL_TEXTURE_2D, GLuint(m_PrevTexture)); CHECK_GL_ERROR;

    _glUseProgram(m_PrevProgramObject); CHECK_GL_ERROR;
    
    _glBindVertexArray(GLuint(m_PrevVArray)); CHECK_GL_ERROR;
...
void CTwGraphOpenGLCore::BuildText(void *_TextObj, const std::string *_TextLines, color32 *_LineColors, color32 *_LineBgColors, int _NbLines, const CTexFont *_Font, int _Sep, int _BgWidth)
{
    ...
    for( int Line=0; Line<_NbLines; ++Line )
    {
        ...
        if( _LineColors!=NULL )
	  LineColor = (_LineColors[Line]&0xff00ff00) | GLubyte(_LineColors[Line]>>16) | (GLuint(GLubyte(_LineColors[Line])<<16));
        ...
        if( _BgWidth>0 )
        {
            ...
            {
	      color32 LineBgColor = (_LineBgColors[Line]&0xff00ff00) | GLubyte(_LineBgColors[Line]>>16) | (GLuint(GLubyte(_LineBgColors[Line])<<16));
...
void CTwGraphOpenGLCore::DrawText(void *_TextObj, int _X, int _Y, color32 _Color, color32 _BgColor)
{
    ...
    if( (_BgColor!=0 || TextObj->m_BgColors.size()==TextObj->m_BgVerts.size()) && TextObj->m_BgVerts.size()>=4 )
    {
        ...
        _glBindBuffer(GL_ARRAY_BUFFER, m_TriVertices);
        _glBufferSubData(GL_ARRAY_BUFFER, 0, GLsizeiptr(numBgVerts*sizeof(Vec2)), &(TextObj->m_BgVerts[0]));
        ...
        {
            _glBindBuffer(GL_ARRAY_BUFFER, m_TriColors);
            _glBufferSubData(GL_ARRAY_BUFFER, 0, GLsizeiptr(numBgVerts*sizeof(color32)), &(TextObj->m_BgColors[0]));
            _glVertexAttribPointer(1, GL_BGRA, GL_UNSIGNED_BYTE, GL_TRUE, 0, NULL);
            _glEnableVertexAttribArray(1);
    ...
    if( TextObj->m_TextVerts.size()>=4 )
    {
        ...
        _glBindBuffer(GL_ARRAY_BUFFER, m_TriVertices);
        _glBufferSubData(GL_ARRAY_BUFFER, 0,  GLsizeiptr(numTextVerts*sizeof(Vec2)), &(TextObj->m_TextVerts[0]));
        _glVertexAttribPointer(0, 2, GL_FLOAT, GL_TRUE, 0, NULL);
        _glEnableVertexAttribArray(0);

        _glBindBuffer(GL_ARRAY_BUFFER, m_TriUVs);
        _glBufferSubData(GL_ARRAY_BUFFER, 0,  GLsizeiptr(numTextVerts*sizeof(Vec2)), &(TextObj->m_TextUVs[0]));
        ...
        {
            _glBindBuffer(GL_ARRAY_BUFFER, m_TriColors);
            _glBufferSubData(GL_ARRAY_BUFFER, 0,  GLsizeiptr(numTextVerts*sizeof(color32)), &(TextObj->m_Colors[0]));
...
void CTwGraphOpenGLCore::DrawTriangles(int _NumTriangles, int *_Vertices, color32 *_Colors, Cull _CullMode)
{
    ...
    size_t numVerts = size_t(3*_NumTriangles);
    if( numVerts > m_TriBufferSize )
        ResizeTriBuffers(numVerts + 2048);
  
    _glBindBuffer(GL_ARRAY_BUFFER, m_TriVertices);
    _glBufferSubData(GL_ARRAY_BUFFER, 0,  GLsizeiptr(numVerts*2*sizeof(int)), _Vertices);
    _glVertexAttribPointer(0, 2, GL_INT, GL_FALSE, 0, NULL);
    _glEnableVertexAttribArray(0);

    _glBindBuffer(GL_ARRAY_BUFFER, m_TriColors);
    _glBufferSubData(GL_ARRAY_BUFFER, 0,  GLsizeiptr(numVerts*sizeof(color32)), _Colors);
    _glVertexAttribPointer(1, GL_BGRA, GL_UNSIGNED_BYTE, GL_TRUE, 0, NULL);
    _glEnableVertexAttribArray(1);
    ...
    // Reset states
    _glCullFace(GLuint(prevCullFaceMode));
    _glFrontFace(GLuint(prevFrontFace));
    if( prevCullEnable )
        _glEnable(GL_CULL_FACE);
    else
        _glDisable(GL_CULL_FACE);
