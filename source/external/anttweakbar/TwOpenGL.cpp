...
//#endif
    ...
    */
    ...
    for( GLuint i=0; GLint(i)<m_MaxClipPlanes; ++i )
    ...
    if( _glDisableVertexAttribArray!=NULL )
    {
        ...
        for(GLuint i=0; GLint(i)<maxVertexAttribs; i++)
        {
...
void CTwGraphOpenGL::EndDraw()
{
    ...
    _glBindTexture(GL_TEXTURE_2D, GLuint(m_PrevTexture));
    /*if( _glBindVertexArray!=NULL )
      _glBindVertexArray(m_PrevVertexArray);*/
    if( _glBindBufferARB!=NULL )
    {
      _glBindBufferARB(GL_ARRAY_BUFFER_ARB, GLuint(m_PrevArrayBufferARB));
      _glBindBufferARB(GL_ELEMENT_ARRAY_BUFFER_ARB, GLuint(m_PrevElementArrayBufferARB));
    }
    if( _glBindProgramARB!=NULL )
    {
    ...
    if( _glBlendEquation!=NULL )
      _glBlendEquation(GLuint(m_PrevBlendEquation));
    if( _glBlendEquationSeparate!=NULL )
      _glBlendEquationSeparate(GLuint(m_PrevBlendEquationRGB), GLuint(m_PrevBlendEquationAlpha));
    if( _glBlendFuncSeparate!=NULL )
      _glBlendFuncSeparate(GLuint(m_PrevBlendSrcRGB), GLuint(m_PrevBlendDstRGB), GLuint(m_PrevBlendSrcAlpha), GLuint(m_PrevBlendDstAlpha));
    
    _glPolygonMode(GL_FRONT, GLuint(m_PrevPolygonMode[0]));
    _glPolygonMode(GL_BACK, GLuint(m_PrevPolygonMode[1]));
    ...
    if( _glActiveTextureARB )
    {
        ...
        _glActiveTextureARB(GLuint(m_PrevActiveTextureARB));
        ...
        _glClientActiveTextureARB(GLuint(m_PrevClientActiveTextureARB));
    }
...
void CTwGraphOpenGL::ChangeViewport(int _X0, int _Y0, int _Width, int _Height, int _OffsetX, int _OffsetY)
{
    if( _Width>0 && _Height>0 )
    {
        ...
        _glMatrixMode(GLuint(matrixMode));
    }
}
