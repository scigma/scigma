...
void CTwVarAtom::ValueToString(string *_Str) const
{
    ...
    else if( m_Type==TW_TYPE_HELP_STRUCT )
    {
        ...
        {
	  if( g_TwMgr->m_Structs[size_t(idx)].m_Name.length()>0 )
	    (*_Str) = '{' + g_TwMgr->m_Structs[size_t(idx)].m_Name + '}';
            else
                (*_Str) = "{struct}";
        }
        return;
...
#pragma clang diagnostic pop
	...
	*/
        ...
        else if( IsCSStringType(m_Type) )
	  {
		  ...
		  g_TwMgr->m_CSStringBuffer.resize(size_t(n)+32);
            ...
            */
            {
                ...
                if( this==ThisVar && Parent!=NULL && Parent->m_StructValuePtr!=NULL )
                {
                    ...
                    if( Idx>=0 && Idx<(int)g_TwMgr->m_Structs.size() )
                    {
                        ...
                        for( size_t im=0; im<g_TwMgr->m_Structs[size_t(Idx)].m_Members.size(); ++im )
                        {
			  size_t ml = g_TwMgr->m_Structs[size_t(Idx)].m_Members[im].m_Name.length();
			  if( nl>=ml && strcmp(g_TwMgr->m_Structs[size_t(Idx)].m_Members[im].m_Name.c_str(), m_Name.c_str()+(nl-ml))==0 )
                            {
                                // TODO: would have to be applied to other vars already created
                                if( _AttribID==V_LABEL )
                                {
				  g_TwMgr->m_Structs[size_t(Idx)].m_Members[im].m_Label = _Value;
//                                    m_Label = _Value;
                                }
                                else // V_HELP
				  g_TwMgr->m_Structs[size_t(Idx)].m_Members[im].m_Help = _Value;
                                break;
                            }
                        }
                    }
        ...
        if( _Value!=NULL && strlen(_Value)>0 ) // do not check ReadOnly here.
        {
            if( !( m_Type==TW_TYPE_BUTTON || IsCustom() ) ) // || (m_Type>=TW_TYPE_CUSTOM_BASE && m_Type<TW_TYPE_CUSTOM_BASE+(int)g_TwMgr->m_Customs.size()) ) )
            {
			  ...
			  str.resize(size_t(n)-1);
                        ...
                        {
                            if( n>1 )
			      strncpy((char *)m_Ptr, str.c_str(), size_t(n)-1);
                            ((char *)m_Ptr)[n-1] = '\0';
                            _Bar->NotUpToDate();
                            return 1;
                        }
    ...
    case TW_TYPE_UINT32:
        {
	      ...
	      v =((unsigned int)(_Step)*m_Val.m_UInt32.m_Step + v);
    ...
    case VG_TYPEID:
        {
            ...
            if( _Value!=NULL && sscanf(_Value, "%d", &type)==1 )
            {
                ...
                if( idx>=0 && idx<(int)g_TwMgr->m_Structs.size() )
                {
		  m_SummaryCallback   = g_TwMgr->m_Structs[size_t(idx)].m_SummaryCallback;
		  m_SummaryClientData = g_TwMgr->m_Structs[size_t(idx)].m_SummaryClientData;
...
#pragma clang diagnostic ignored "-Wswitch-enum"
    ...
    default:
	  ...
	  return size_t(TW_CSSTRING_SIZE(size_t(_Type)));
...
int CTwBar::SetAttrib(int _AttribID, const char *_Value)
{
    ...
    case BAR_ALPHA:
        if( _Value && strlen(_Value)>0 )
        {
	      ...
	      m_Color = ((unsigned int)(alpha)<<24) | (m_Color & 0xffffff);
            else
            {
                g_TwMgr->SetLastError(g_ErrBadValue);
                return 0;
...
void CTwBar::ListLabels(vector<string>& _Labels, vector<color32>& _Colors, vector<color32>& _BgColors, bool *_HasBgColors, const CTexFont *_Font, int _AtomWidthMax, int _GroupWidthMax)
{
    ...
    for( int h=0; h<nh; ++h )
    {
      Len = (int)m_HierTags[size_t(h)].m_Var->m_Label.length();
        if( Len>0 )
	  Text = (const unsigned char *)(m_HierTags[size_t(h)].m_Var->m_Label.c_str());
        else
        {
	  Text = (const unsigned char *)(m_HierTags[size_t(h)].m_Var->m_Name.c_str());
	  Len = (int)m_HierTags[size_t(h)].m_Var->m_Name.length();
        }
        x = 0;
        Etc = 0;
        _Labels.push_back("");  // add a new text line
        if( !m_HierTags[size_t(h)].m_Var->IsGroup() && static_cast<const CTwVarAtom *>(m_HierTags[size_t(h)].m_Var)->m_Type==TW_TYPE_BUTTON && static_cast<const CTwVarAtom *>(m_HierTags[size_t(h)].m_Var)->m_ReadOnly && static_cast<const CTwVarAtom *>(m_HierTags[size_t(h)].m_Var)->m_Val.m_Button.m_Callback!=NULL )
            _Colors.push_back(m_ColValTextRO); // special case for read-only buttons
        else
            _Colors.push_back(m_HierTags[size_t(h)].m_Var->m_ColorPtr!=NULL ? *(m_HierTags[size_t(h)].m_Var->m_ColorPtr) : COLOR32_WHITE);
        color32 bg = m_HierTags[size_t(h)].m_Var->m_BgColorPtr!=NULL ? *(m_HierTags[size_t(h)].m_Var->m_BgColorPtr) : 0;
        _BgColors.push_back(bg);
        if( _HasBgColors!=NULL && bg!=0 )
            *_HasBgColors = true;
        bool IsCustom = m_HierTags[size_t(h)].m_Var->IsCustom(); // !m_HierTags[size_t(h)].m_Var->IsGroup() && (static_cast<const CTwVarAtom *>(m_HierTags[size_t(h)].m_Var)->m_Type>=TW_TYPE_CUSTOM_BASE && static_cast<const CTwVarAtom *>(m_HierTags[size_t(h)].m_Var)->m_Type<TW_TYPE_CUSTOM_BASE+(int)g_TwMgr->m_Customs.size());
        if( !IsCustom )
        {
            string& CurrentLabel = _Labels[_Labels.size()-1];
            if( m_HierTags[size_t(h)].m_Var->IsGroup() && static_cast<const CTwVarGroup *>(m_HierTags[size_t(h)].m_Var)->m_SummaryCallback==NULL )
                WidthMax = _GroupWidthMax;
            else if( !m_HierTags[size_t(h)].m_Var->IsGroup() && static_cast<const CTwVarAtom *>(m_HierTags[size_t(h)].m_Var)->m_Type==TW_TYPE_BUTTON )
            {
                if( static_cast<const CTwVarAtom *>(m_HierTags[size_t(h)].m_Var)->m_Val.m_Button.m_Callback==NULL )
                    WidthMax = _GroupWidthMax;
            ...
            //  WidthMax = _AtomWidthMax - IncrBtnWidth(m_Font->m_CharHeight);
                ...
                WidthMax = _AtomWidthMax;
            if( Space>0 )
                for( s=0; s<m_HierTags[size_t(h)].m_Level*LevelSpace; s+=Space )
                {
                    CurrentLabel += ' ';
                    x += Space;
                }
            if( x+(NbEtc+2)*_Font->m_CharWidth[(int)'.']<WidthMax || m_HierTags[size_t(h)].m_Var->m_DontClip)
                for( i=0; i<Len; ++i )
                {
                    ch = (Etc==0) ? Text[i] : '.';
                    CurrentLabel += char(ch);
                    ...
                    else if( i<Len-2 && x+(NbEtc+2)*_Font->m_CharWidth[(int)'.']>=WidthMax && !(m_HierTags[size_t(h)].m_Var->m_DontClip))
                        Etc = 1;
                }       
        }
    }
...
void CTwBar::ListValues(vector<string>& _Values, vector<color32>& _Colors, vector<color32>& _BgColors, const CTexFont *_Font, int _WidthMax)
{
    ...
    bool HasBgColor;
    bool AcceptEdit;
    size_t SummaryMaxLength = size_t(max(_WidthMax/_Font->m_CharWidth[(int)'I'], 4));
#pragma clang diagnostic push
...
#pragma clang diagnostic pop
    ...
    int nh = (int)m_HierTags.size();
    for( int h=0; h<nh; ++h )
        if( !m_HierTags[size_t(h)].m_Var->IsGroup() || m_IsHelpBar 
            || (m_HierTags[size_t(h)].m_Var->IsGroup() && static_cast<const CTwVarGroup *>(m_HierTags[size_t(h)].m_Var)->m_SummaryCallback!=NULL) )
        {
            ReadOnly = true;
            IsMax = false;
            IsMin = false;
            ...
            HasBgColor = true;
            AcceptEdit = false;
            if( !m_HierTags[size_t(h)].m_Var->IsGroup() )
            {
                Atom = static_cast<const CTwVarAtom *>(m_HierTags[size_t(h)].m_Var);
            ...
            else if(m_HierTags[size_t(h)].m_Var->IsGroup() && static_cast<const CTwVarGroup *>(m_HierTags[size_t(h)].m_Var)->m_SummaryCallback!=NULL)
            {
                const CTwVarGroup *Grp = static_cast<const CTwVarGroup *>(m_HierTags[size_t(h)].m_Var);
            ...
            else if( m_HierTags[size_t(h)].m_Var->IsGroup() )
            {
                const CTwVarGroup *Grp = static_cast<const CTwVarGroup *>(m_HierTags[size_t(h)].m_Var);
...
static int ClampText(string& _Text, const CTexFont *_Font, int _WidthMax)
{
    ...
    for( i=0; i<Len; ++i )
    {
      ch = (unsigned char)(_Text.at(size_t(i)));
...
void CTwBar::Update()
{
    ...
    if( !m_IsMinimized )
    {
	  ...
	  m_HierTags.resize(size_t(NbLines)); // remove the last dummy tag
    ...
    if( !m_IsMinimized )
    {
        ...
        m_DrawClickBtn    = ( m_VarX2-m_VarX1>4*IncrBtnWidth(m_Font->m_CharHeight)
                              ...
                              && m_HierTags[size_t(m_HighlightedLine)].m_Var!=NULL 
                              && !m_HierTags[size_t(m_HighlightedLine)].m_Var->IsGroup()
                              && !static_cast<CTwVarAtom *>(m_HierTags[size_t(m_HighlightedLine)].m_Var)->m_ReadOnly
                              && (    static_cast<CTwVarAtom *>(m_HierTags[size_t(m_HighlightedLine)].m_Var)->m_Type==TW_TYPE_BUTTON ));
                            //     || static_cast<CTwVarAtom *>(m_HierTags[size_t(m_HighlightedLine)].m_Var)->m_Type==TW_TYPE_BOOLCPP
                            //     || static_cast<CTwVarAtom *>(m_HierTags[size_t(m_HighlightedLine)].m_Var)->m_Type==TW_TYPE_BOOL8
                            //     || static_cast<CTwVarAtom *>(m_HierTags[size_t(m_HighlightedLine)].m_Var)->m_Type==TW_TYPE_BOOL16
                            //     || static_cast<CTwVarAtom *>(m_HierTags[size_t(m_HighlightedLine)].m_Var)->m_Type==TW_TYPE_BOOL32 ));
        ...
        m_DrawIncrDecrBtn = ( m_VarX2-m_VarX1>5*IncrBtnWidth(m_Font->m_CharHeight)
                              ...
                              && static_cast<CTwVarAtom *>(m_HierTags[size_t(m_HighlightedLine)].m_Var)->m_Type!=TW_TYPE_BUTTON
                              && !static_cast<CTwVarAtom *>(m_HierTags[size_t(m_HighlightedLine)].m_Var)->m_ReadOnly
                              && !static_cast<CTwVarAtom *>(m_HierTags[size_t(m_HighlightedLine)].m_Var)->m_NoSlider 
                              && !(m_EditInPlace.m_Active && m_EditInPlace.m_Var==m_HierTags[size_t(m_HighlightedLine)].m_Var) );
        ...
        m_DrawListBtn     = ( m_VarX2-m_VarX1>2*IncrBtnWidth(m_Font->m_CharHeight)
                              ...
                              && m_HierTags[size_t(m_HighlightedLine)].m_Var!=NULL 
                              && !m_HierTags[size_t(m_HighlightedLine)].m_Var->IsGroup()
                              && IsEnumType(static_cast<CTwVarAtom *>(m_HierTags[size_t(m_HighlightedLine)].m_Var)->m_Type)
                              && !static_cast<CTwVarAtom *>(m_HierTags[size_t(m_HighlightedLine)].m_Var)->m_ReadOnly );
        ...
        */
        ...
        string Shortcut;
        m_ShortcutLine = -1;
        if( m_HighlightedLine>=0 && m_HighlightedLine<(int)m_HierTags.size() && m_HierTags[size_t(m_HighlightedLine)].m_Var!=NULL && !m_HierTags[size_t(m_HighlightedLine)].m_Var->IsGroup() )
        {
            const CTwVarAtom *Atom = static_cast<const CTwVarAtom *>(m_HierTags[size_t(m_HighlightedLine)].m_Var);
...
void CTwBar::DrawHierHandle()
{
    ...
    if( !m_IsPopupList )
    {
        ...
        for( h=0; h<nh; ++h )
        {
            ...
            if( m_HierTags[size_t(h)].m_Var->IsGroup() )
                Grp = static_cast<CTwVarGroup *>(m_HierTags[size_t(h)].m_Var);
            else
                Grp = NULL;

            int dx = m_HierTags[size_t(h)].m_Level * (x2-x0);
            ...
            if( Grp )
            {
                ...
                {
                    color32 cb = (Grp->m_StructType==TW_TYPE_HELP_STRUCT) ? m_ColStructBg : m_ColGrpBg;
                    //int decal = m_Font->m_CharHeight/2-2+2*m_HierTags[size_t(h)].m_Level;
                    //if( decal>m_Font->m_CharHeight-3 )
                    //  decal = m_Font->m_CharHeight-3;
                    int margin = dx; //m_Font->m_CharWidth[(int)' ']*m_HierTags[size_t(h)].m_Level;
            ...
            else if( static_cast<CTwVarAtom *>(m_HierTags[size_t(h)].m_Var)->m_Type==TW_TYPE_HELP_GRP && m_ColHelpBg!=0 )
                Gr->DrawRect(m_PosX+m_VarX0+m_HierTags[size_t(h)].m_Var->m_LeftMargin, y0+m_HierTags[size_t(h)].m_Var->m_TopMargin, m_PosX+m_VarX2, y0+m_Font->m_CharHeight-1, m_ColHelpBg);
            //else if( static_cast<CTwVarAtom *>(m_HierTags[size_t(h)].m_Var)->m_Type==TW_TYPE_HELP_HEADER && m_ColHelpBg!=0 )
            //  Gr->DrawRect(m_PosX+m_VarX0+m_HierTags[size_t(h)].m_Var->m_LeftMargin, y0+m_HierTags[size_t(h)].m_Var->m_TopMargin, m_PosX+m_VarX2, y0+m_Font->m_CharHeight-1, m_ColHelpBg);
            /*
            else if( static_cast<CTwVarAtom *>(m_HierTags[size_t(h)].m_Var)->m_Type==TW_TYPE_BUTTON && m_ColBtn!=0 )
            {
                // draw button
...
void CTwBar::Draw(int _DrawPart)
{
    ...
    if( !m_IsMinimized )
    {
        ...
        if( _DrawPart&DRAW_CONTENT )
        {
            ...
            if( m_HighlightedLine>=0 && m_HighlightedLine<(int)m_HierTags.size() && m_HierTags[size_t(m_HighlightedLine)].m_Var!=NULL
                && (m_HierTags[size_t(m_HighlightedLine)].m_Var->IsGroup() 
                    || (!static_cast<CTwVarAtom *>(m_HierTags[size_t(m_HighlightedLine)].m_Var)->m_ReadOnly && !m_IsHelpBar 
                        && !m_HierTags[size_t(m_HighlightedLine)].m_Var->IsCustom() ) ) ) // !(static_cast<CTwVarAtom *>(m_HierTags[size_t(m_HighlightedLine)].m_Var)->m_Type>=TW_TYPE_CUSTOM_BASE && static_cast<CTwVarAtom *>(m_HierTags[size_t(m_HighlightedLine)].m_Var)->m_Type<TW_TYPE_CUSTOM_BASE+(int)g_TwMgr->m_Customs.size()))) )
            {
                int y0 = m_PosY + m_VarY0 + m_HighlightedLine*(m_Font->m_CharHeight+m_LineSep);
                Gr->DrawRect(m_PosX+LevelSpace+6+LevelSpace*m_HierTags[size_t(m_HighlightedLine)].m_Level, y0+1, m_PosX+m_VarX2, y0+m_Font->m_CharHeight-1+m_LineSep-1, m_ColHighBg0, m_ColHighBg0, m_ColHighBg1, m_ColHighBg1);
                int eps = (g_TwMgr->m_GraphAPI==TW_OPENGL || g_TwMgr->m_GraphAPI==TW_OPENGL_CORE) ? 1 : 0;
                if( !m_EditInPlace.m_Active )
                    Gr->DrawLine(m_PosX+LevelSpace+6+LevelSpace*m_HierTags[size_t(m_HighlightedLine)].m_Level, y0+m_Font->m_CharHeight+m_LineSep-1+eps, m_PosX+m_VarX2, y0+m_Font->m_CharHeight+m_LineSep-1+eps, m_ColUnderline);
            }
            else if( m_HighlightedLine>=0 && m_HighlightedLine<(int)m_HierTags.size() && !m_HierTags[size_t(m_HighlightedLine)].m_Var->IsGroup() )
            {
                int y0 = m_PosY + m_VarY0 + m_HighlightedLine*(m_Font->m_CharHeight+m_LineSep);
                color32 col = ColorBlend(m_ColHighBg0, m_ColHighBg1, 0.5f);
                CTwVarAtom *Atom = static_cast<CTwVarAtom *>(m_HierTags[size_t(m_HighlightedLine)].m_Var);
                if( !Atom->IsCustom() // !(Atom->m_Type>=TW_TYPE_CUSTOM_BASE && Atom->m_Type<TW_TYPE_CUSTOM_BASE+(int)g_TwMgr->m_Customs.size()) 
                    ...
                    Gr->DrawRect(m_PosX+LevelSpace+6+LevelSpace*m_HierTags[size_t(m_HighlightedLine)].m_Level, y0+1, m_PosX+m_VarX2, y0+m_Font->m_CharHeight-1+m_LineSep-1, col);
                else
                    Gr->DrawRect(m_PosX+LevelSpace+6+LevelSpace*m_HierTags[size_t(m_HighlightedLine)].m_Level, y0+1, m_PosX+LevelSpace+6+LevelSpace*m_HierTags[size_t(m_HighlightedLine)].m_Level+4, y0+m_Font->m_CharHeight-1+m_LineSep-1, col);
            }
	...
	/*
                ...
                else if( static_cast<CTwVarAtom *>(m_HierTags[size_t(h)].m_Var)->m_Type==TW_TYPE_BUTTON && !m_IsPopupList )
                {
                    // draw button
                    int cbx0, cbx1;
                    if( m_ButtonAlign == BUTTON_ALIGN_LEFT )
                    ...
                    if( !static_cast<CTwVarAtom *>(m_HierTags[size_t(h)].m_Var)->m_ReadOnly )
                    {
                    ...
                    else if( static_cast<CTwVarAtom *>(m_HierTags[size_t(h)].m_Var)->m_Val.m_Button.m_Callback!=NULL )
                    {
                        Gr->DrawRect(cbx0+1, cby0+1, cbx1+1, cby1+1, m_ColBtn);
                    }
                    else if( static_cast<CTwVarAtom *>(m_HierTags[size_t(h)].m_Var)->m_Val.m_Button.m_Separator==1 )
                    {
                        int LLevelSpace = max(m_Font->m_CharHeight-6, 4); // space used by DrawHierHandles
                        Gr->DrawLine(m_PosX+m_VarX0+m_HierTags[size_t(h)].m_Level*LLevelSpace, yh+m_Font->m_CharHeight/2, m_PosX+m_VarX2, yh+m_Font->m_CharHeight/2, m_ColSeparator );
                    }
                }
                else if( m_HierTags[size_t(h)].m_Var->IsCustom() ) //static_cast<CTwVarAtom *>(m_HierTags[size_t(h)].m_Var)->m_Type>=TW_TYPE_CUSTOM_BASE && static_cast<CTwVarAtom *>(m_HierTags[size_t(h)].m_Var)->m_Type<TW_TYPE_CUSTOM_BASE+(int)g_TwMgr->m_Customs.size() )
                {   // record custom types
                    CTwMgr::CMemberProxy *mProxy = static_cast<CTwVarAtom *>(m_HierTags[size_t(h)].m_Var)->m_Val.m_Custom.m_MemberProxy;
                    if( mProxy!=NULL && mProxy->m_StructProxy!=NULL )
                    {
                        CustomMap::iterator it = m_CustomRecords.find(mProxy->m_StructProxy);
                        int xMin = m_PosX + m_VarX0 + m_HierTags[size_t(h)].m_Level*LevelSpace;
            ...
            if( m_DrawHandles && !m_IsPopupList )
            {
                ...
                if( (m_DrawIncrDecrBtn || m_DrawClickBtn || m_DrawListBtn || m_DrawBoolBtn || m_DrawRotoBtn) && m_HighlightedLine>=0 && m_HighlightedLine<(int)m_HierTags.size() )
                {
                    ...
                    if( m_DrawIncrDecrBtn )
                    {
                        bool IsMin = false;
                        bool IsMax = false;
                        if( !m_HierTags[size_t(m_HighlightedLine)].m_Var->IsGroup() )
                        {
                            const CTwVarAtom *Atom = static_cast<const CTwVarAtom *>(m_HierTags[size_t(m_HighlightedLine)].m_Var);
                            double v, vmin, vmax;
                            v = Atom->ValueToDouble();
    ...
    if( !m_IsMinimized )
    {
        ...
        {   
            ...
            {   // mouse over var line
                ...
                if( m_HighlightedLine<0 || m_HierTags[size_t(m_HighlightedLine)].m_Var==NULL || m_HierTags[size_t(m_HighlightedLine)].m_Var->IsGroup() )
                    ANT_SET_CURSOR(Arrow);
                else
                {
                    if( !m_HierTags[size_t(m_HighlightedLine)].m_Var->IsGroup() && static_cast<CTwVarAtom *>(m_HierTags[size_t(m_HighlightedLine)].m_Var)->m_NoSlider )
                    {
                        if( static_cast<CTwVarAtom *>(m_HierTags[size_t(m_HighlightedLine)].m_Var)->m_ReadOnly && !m_IsHelpBar 
                            && !(static_cast<CTwVarAtom *>(m_HierTags[size_t(m_HighlightedLine)].m_Var)->m_Type==TW_TYPE_BUTTON && static_cast<CTwVarAtom *>(m_HierTags[size_t(m_HighlightedLine)].m_Var)->m_Val.m_Button.m_Callback==NULL) )
                    ...
                    else if( !m_HierTags[size_t(m_HighlightedLine)].m_Var->IsGroup() && static_cast<CTwVarAtom *>(m_HierTags[size_t(m_HighlightedLine)].m_Var)->m_ReadOnly )
                    {
                        if( !m_IsHelpBar )
                            ANT_SET_CURSOR(No);
                        else
        ...
        {
            if( m_MouseDragVar && m_HighlightedLine>=0 && m_HighlightedLine<(int)m_HierTags.size() && m_HierTags[size_t(m_HighlightedLine)].m_Var && !m_HierTags[size_t(m_HighlightedLine)].m_Var->IsGroup() )
            {
                /*
                CTwVarAtom *Var = static_cast<CTwVarAtom *>(m_HierTags[size_t(m_HighlightedLine)].m_Var);
                ...
                */
                ...
                // move rotoslider
                if( !static_cast<CTwVarAtom *>(m_HierTags[size_t(m_HighlightedLine)].m_Var)->m_NoSlider )
                    RotoOnMouseMove(_X, _Y);

                if( static_cast<CTwVarAtom *>(m_HierTags[size_t(m_HighlightedLine)].m_Var)->m_ReadOnly )
                    ANT_SET_CURSOR(No);
                else if( static_cast<CTwVarAtom *>(m_HierTags[size_t(m_HighlightedLine)].m_Var)->m_NoSlider )
                {
                    ANT_SET_CURSOR(Arrow);
                    CustomArea = true;
                }
...
bool CTwBar::MouseButton(ETwMouseButtonID _Button, bool _Pressed, int _X, int _Y)
{
    ...
    if( !m_IsMinimized )
    {
        ...
        if( _Button==TW_MOUSE_LEFT && m_HighlightedLine>=0 && m_HighlightedLine<(int)m_HierTags.size() && m_HierTags[size_t(m_HighlightedLine)].m_Var )
        {
            ...
            if( m_HierTags[size_t(m_HighlightedLine)].m_Var->IsGroup() )
            {
                ...
                {
                    CTwVarGroup *Grp = static_cast<CTwVarGroup *>(m_HierTags[size_t(m_HighlightedLine)].m_Var);
            ...
            {
                static_cast<CTwVarAtom *>(m_HierTags[size_t(m_HighlightedLine)].m_Var)->Increment(1);
                if( g_TwMgr==NULL ) // Mgr might have been destroyed by the client inside a callback call
                    return 1;
                NotUpToDate();
            }
...
#pragma clang diagnostic pop
            ...
            else if ( !_Pressed && m_MouseDragVar )
            {
                ...
                if( !m_VarHasBeenIncr && !static_cast<CTwVarAtom *>(m_HierTags[size_t(m_HighlightedLine)].m_Var)->m_ReadOnly )
                {
    ...
    if( _Key>0 && _Key<TW_KEY_LAST )
    {
        ...
        */
        ...
        else
        {
            ...
            else if( BarActive && m_HighlightedLine>=0 && m_HighlightedLine<(int)m_HierTags.size() && m_HierTags[size_t(m_HighlightedLine)].m_Var )
            {
                if( _Key==TW_KEY_RIGHT )
                {
                    if( !m_HierTags[size_t(m_HighlightedLine)].m_Var->IsGroup() ) 
                    {
                        CTwVarAtom *AAtom = static_cast<CTwVarAtom *>(m_HierTags[size_t(m_HighlightedLine)].m_Var);
                        bool Accept = !AAtom->m_NoSlider || AAtom->m_Type==TW_TYPE_BUTTON 
                                      || AAtom->m_Type==TW_TYPE_BOOL8 || AAtom->m_Type==TW_TYPE_BOOL16 || AAtom->m_Type==TW_TYPE_BOOL32 || AAtom->m_Type==TW_TYPE_BOOLCPP
                                      || IsEnumType(AAtom->m_Type);
                        if( !AAtom->IsReadOnly() && !m_IsPopupList && Accept )
                        {
                            AAtom->Increment(+1);
                            if( g_TwMgr==NULL ) // Mgr might have been destroyed by the client inside a callback call
                                return 1;
                ...
                else if( _Key==TW_KEY_LEFT )
                {
                    if( !m_HierTags[size_t(m_HighlightedLine)].m_Var->IsGroup() ) 
                    {
                        ...
                        bool Accept = !AAtom->m_NoSlider || AAtom->m_Type==TW_TYPE_BUTTON 
                                      || AAtom->m_Type==TW_TYPE_BOOL8 || AAtom->m_Type==TW_TYPE_BOOL16 || AAtom->m_Type==TW_TYPE_BOOL32 || AAtom->m_Type==TW_TYPE_BOOLCPP
                                      || IsEnumType(AAtom->m_Type);
                        if( !AAtom->IsReadOnly() && Accept && !m_IsPopupList )
                        {
                            AAtom->Increment(-1);
                            if( g_TwMgr==NULL ) // Mgr might have been destroyed by the client inside a callback call
                                return 1;
                ...
                else if( _Key==TW_KEY_RETURN )
                {
                    if( !m_HierTags[size_t(m_HighlightedLine)].m_Var->IsGroup() ) 
                    {
                        ...
                        if( !AAtom->IsReadOnly() )
                        {
                            if( AAtom->m_Type==TW_TYPE_BUTTON || AAtom->m_Type==TW_TYPE_BOOLCPP 
                                || AAtom->m_Type==TW_TYPE_BOOL8 || AAtom->m_Type==TW_TYPE_BOOL16 || AAtom->m_Type==TW_TYPE_BOOL32 )
                            {
...
{
    ITwGraph *Gr = g_TwMgr->m_Graph;
    if( Gr==NULL || !Gr->IsDrawing() || _Radius==0 || !(_StartAngleDeg>_EndAngleDeg||_StartAngleDeg<_EndAngleDeg ))
        return;
...
{
    assert(m_Roto.m_Var!=NULL);
    if( _Val>m_Roto.m_CurrentValue||_Val<m_Roto.m_CurrentValue )
    {
...
{
    CPoint p(_X, _Y);
    if( !m_Roto.m_Active && m_HighlightedLine>=0 && m_HighlightedLine<(int)m_HierTags.size() && m_HierTags[size_t(m_HighlightedLine)].m_Var && !m_HierTags[size_t(m_HighlightedLine)].m_Var->IsGroup() )
    {
        m_Roto.m_Var = static_cast<CTwVarAtom *>(m_HierTags[size_t(m_HighlightedLine)].m_Var);
        int y = m_PosY + m_VarY0 + m_HighlightedLine*(m_Font->m_CharHeight+m_LineSep) + m_Font->m_CharHeight/2;
...
void CTwBar::EditInPlaceDraw()
{
    ...
    if( SubstrWidth>=m_EditInPlace.m_Width )
        --LastChar;
    string Substr = m_EditInPlace.m_String.substr(size_t(m_EditInPlace.m_FirstChar), size_t(LastChar-m_EditInPlace.m_FirstChar));
    ...
    if( m_EditInPlace.m_CaretPos>m_EditInPlace.m_SelectionStart )
    {
        ...
        StrSelected = m_EditInPlace.m_String.substr(size_t(FirstSel),size_t( LastSel-FirstSel));
    }
    else
    {
...
void CTwBar::EditInPlaceEnd(bool _Commit)
{
    if( _Commit && m_EditInPlace.m_Active && m_EditInPlace.m_Var!=NULL )
    {
        ...
        else if( IsCSStringType(m_EditInPlace.m_Var->m_Type) )
        {
		  ...
		  m_EditInPlace.m_String.resize(size_t(n)-1);
                if( m_EditInPlace.m_Var->m_SetCallback!=NULL )
                ...
                {
                    if( n>1 )
		      strncpy((char *)m_EditInPlace.m_Var->m_Ptr, m_EditInPlace.m_String.c_str(), size_t(n)-1);
...
bool CTwBar::EditInPlaceKeyPressed(int _Key, int _Modifiers)
{
    ...
    default:
        ...
        else if( _Key>=32 && _Key<=255 )
        {
            if( !EditInPlaceIsReadOnly() && m_EditInPlace.m_CaretPos>=0 && m_EditInPlace.m_CaretPos<=(int)m_EditInPlace.m_String.length() )
            {
                ...
                m_EditInPlace.m_String.insert(size_t(m_EditInPlace.m_CaretPos), Str);
    ...
    if( DoCopy )
    {
        string Str = "";
        if( m_EditInPlace.m_CaretPos>m_EditInPlace.m_SelectionStart )
	  Str = m_EditInPlace.m_String.substr(size_t(m_EditInPlace.m_SelectionStart), size_t(m_EditInPlace.m_CaretPos-m_EditInPlace.m_SelectionStart));
        else if( m_EditInPlace.m_CaretPos<m_EditInPlace.m_SelectionStart )
	  Str = m_EditInPlace.m_String.substr(size_t(m_EditInPlace.m_CaretPos), size_t(m_EditInPlace.m_SelectionStart-m_EditInPlace.m_CaretPos));
...
bool CTwBar::EditInPlaceEraseSelect()
{
    ...
    if( !EditInPlaceIsReadOnly() && m_EditInPlace.m_SelectionStart!=m_EditInPlace.m_CaretPos )
    {
        ...
        m_EditInPlace.m_String.erase( size_t(PosMin), size_t(abs(m_EditInPlace.m_CaretPos - m_EditInPlace.m_SelectionStart)) );
