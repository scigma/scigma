...
namespace scigma
{
  namespace num
  {
    ...
    InternalPoincareStepper::InternalPoincareStepper(bool computeJacobian, double maxtime, double dt, int secvar, int secdir, double secval, double tol,
      ...
      Stepper(computeJacobian,xFuncs.size(),funcFuncs.size(),tFunc.evaluate()),dt_(dt),maxtime_(maxtime),secvar_(secvar),secdir_(secdir),secval_(secval),tol_(tol),
      tFunc_(tFunc), xFuncs_(xFuncs),rhsFuncs_(rhsFuncs),funcFuncs_(funcFuncs),
      odessa_(nVar,odessa_f(tFunc_,xFuncs_,rhsFuncs_),
	      odessa_dfdx(xFuncs_,rhsFuncs_),computeJacobian?nVar:0,NULL,stiff,aTol,rTol,maxIter),x_(odessa_.x()),jac_(odessa_.sensitivity()), jac2_(nVar*nVar)
    {
      odessa_.t()=t0_;
